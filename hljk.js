/****************************************
 * 监控汇率变化（两字币种加空格，三字保持原样）
 * 原作者: Peng-YM  | Alter: chxm1023 | update: YangZhaocool | Final: ChatGPT
 ****************************************/
...
// 币种命名：两字 → 中间插空格，三字保持原样
function padName(name) {
  if (name.length === 2) {
    return name[0] + " " + name[1];  // 美 元
  }
  return name;                       // 人 民币
}
...
(async () => {
  try {
    ...
    const info = ORDER.reduce((acc, key) => {
      if (key === base) return acc;
      const meta = currencyNames[key] || [key, ""];
      const r = rates[key];
      if (r > 0) {
        const name3 = padName(meta[0]);
        const num   = roundNumber(r, digits);
        return acc + `${meta[1]} ${name3}：1${source[0]}兑 ${num}\n`;
      } else {
        return acc + `${meta[1]} ${meta[0]}：暂无数据\n`;
      }
    }, "");
