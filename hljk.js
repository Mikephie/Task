...
const base   = "SGD";
const digits = 3;
const NUM_WIDTH = 9;          // 数字列宽（8~10 之间看喜好）
const SHOW_FLAGS = false;     // ← 关：最整齐；开：旗帜放行尾不影响对齐
...
function roundNumber(num, scale) {
  ...
  const arr = ("" + num).split("e"); const sig = (+arr[1] + scale > 0) ? "+" : "";
  return +(Math.round(+arr[0] + "e" + sig + (+arr[1] + scale)) + "e-" + scale);
}
const padName = n => (n.length === 2 ? n + " " : n);      // 两字补空格 → 三字宽
const padNum  = (n,w=NUM_WIDTH) => String(n).padStart(w," ");
...
(async () => {
  try {
    ...
    const info = ORDER.reduce((out, code) => {
      const meta = currencyNames[code];
      if (!meta) return out;
      const val = rates[code];
      if (!(val > 0)) return out;

      const name3 = padName(meta[0]);
      const num   = padNum(roundNumber(val, digits));
      const tailFlag = SHOW_FLAGS ? `  ${meta[1]}` : "";  // 旗帜放行尾
      return out + `${name3}：1${source[0]}兑 ${num}${tailFlag}\n`;
    }, "");

    $.notify(
      `[今日汇率] 基准：${SHOW_FLAGS ? currencyNames[base][1]+" " : ""}${currencyNames[base][0]} (${base})`,
      `⏰ 更新时间：${date || "--"}`,
      `📈 汇率情况：\n${info}`
    );
  } catch (e) {
