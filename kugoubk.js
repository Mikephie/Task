//COOKIE
const kugouAutoVIP = new Env("酷狗概念版自动领取VIP-Eric");
...
if ($request.url) {
  ...
  saveToStorage("headerParams", savedHeaderParams);
  saveToStorage("originalUrl", requestUrl);

  kugouAutoVIP.log("保存的 URL Params:", JSON.stringify(savedUrlParams));
  kugouAutoVIP.log("保存的 Header Params:", JSON.stringify(savedHeaderParams));
  kugouAutoVIP.log("保存的 Original URL:", requestUrl);
  kugouAutoVIP.log("数据已保存到持久化存储-Eric为您提供");
  kugouAutoVIP.msg("获取数据成功", "数据已持久化保存", "");
  kugouAutoVIP.done();
}
